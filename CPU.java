...
import java.util.Arrays;
	...
	public class CPU {
		...
		public int[] outM(int[] inM,int[] instruction,int reset) {
			...
			int ni = NOT.notgate(instruction[0]);
			int[] i = MUX16.mux16(outtM, instruction, ni); 
			int intoA = OR.orgate(instruction[10], ni);
			int[] A = AREGISTER16.register16(prevout, i, intoA);
			int AorM = AND.andgate(instruction[0], instruction[3]);
			int[] AM = MUX16.mux16(A,inM,AorM);
			
			int intoD = AND.andgate(instruction[0],instruction[12]);
			...
			int writeM = AND.andgate(instruction[0], instruction[12]);
			
			int pos  = NOT.notgate(ng);
			int nzr = NOT.notgate(zr);
			int jgt = AND.andgate(instruction[0], instruction[15]);
			int posnzr = AND.andgate(pos, jgt);
			int ld1 = AND.andgate(jgt, posnzr);
			
			int jeq = AND.andgate(instruction[0], instruction[14]);
			int ld2 = AND.andgate(jgt, zr);
			
			int jlt = AND.andgate(instruction[0], instruction[13]);
			int ld3 = AND.andgate(jlt, ng);
			
			int ldt = OR.orgate(ld1, ld2);
			int ld = OR.orgate(ld3, ldt);
